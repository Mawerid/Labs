...
unsigned s_box(unsigned inf_block, int enc);
...
unsigned s_box(unsigned inf_block, int enc) {
    ...
    for(int i = 0; i < LENCHAR; i++) {
        cipher += ind * ((enc) ? (s_box_table[inf_block % FF]) : (s_boxInv_table[inf_block % FF]));
        ind *= FF;
        inf_block >>= 8; // one part of information block
    }
    return cipher;
}
